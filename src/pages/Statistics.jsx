...
function ComboChart({ data, barAKey, barBKey, labelKey, colorA = '#6366f1', colorB = '#10b981', labelA = 'A', labelB = 'B', formatA = v => v, formatB = v => v }) {
  ...
  const BAR_W      = Math.max(Math.min(groupW * 0.32, 24), 3);
  ...
  const linePointsA = data.map((d, i) => d[barAKey] > 0 ? `${groupCenterX(i) - GAP / 2 - BAR_W / 2},${barTopA(d[barAKey])}` : null).filter(Boolean).join(' ');
  const linePointsB = data.map((d, i) => d[barBKey] > 0 ? `${groupCenterX(i) + GAP / 2 + BAR_W / 2},${barTopB(d[barBKey])}` : null).filter(Boolean).join(' ');
    ...
    <div ref={containerRef} className="w-full relative select-none">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-3 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5 font-medium" style={{ color: colorA }}>
          <span className="inline-block w-3 h-3 rounded-sm" style={{ background: colorA }} />{labelA}
        </span>
        <span className="flex items-center gap-1.5 font-medium" style={{ color: colorB }}>
          <span className="inline-block w-3 h-3 rounded-sm" style={{ background: colorB }} />{labelB}
        </span>
      </div>
      ...
      <svg width="100%" viewBox={`0 0 ${width} ${H}`} className="overflow-visible w-full">
        {ticksA.map((t, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={PAD_LEFT - 4} y={y + 3.5} textAnchor="end" fontSize="8" fill={colorA} fillOpacity="0.85">
                {formatA(t)}
              </text>
            </g>
          );
        ...
        {ticksB.map((t, i) => {
          ...
          return (
            <text key={i} x={PAD_LEFT + chartW + 4} y={y + 3.5} textAnchor="start" fontSize="8" fill={colorB} fillOpacity="0.85">
              {formatB(t)}
            </text>
          );
        })}
          ...
          return (
            <g key={i} className="cursor-pointer"
              onMouseEnter={e => {
                ...
                setTip({ x: e.clientX - rect.left, y: e.clientY - rect.top - 48, labelA, labelB, valA: formatA(d[barAKey]), valB: formatB(d[barBKey]), name: d[labelKey] });
              }}
              onMouseLeave={() => setTip(null)}
            >
              {d[barAKey] > 0 && <rect x={xA} y={yA} width={BAR_W} height={hA} rx="2" fill={colorA} fillOpacity="0.85" />}
              {d[barBKey] > 0 && <rect x={xB} y={yB} width={BAR_W} height={hB} rx="2" fill={colorB} fillOpacity="0.85" />}
              <rect x={cx - groupW / 2} y={PAD_TOP} width={groupW} height={chartH} fill="transparent" />
              <text x={cx} y={PAD_TOP + chartH + 12} textAnchor="middle" fontSize="8" fill="currentColor" fillOpacity="0.5">
        ...
        {linePointsA && <polyline points={linePointsA} fill="none" stroke={colorA} strokeWidth="1.5" strokeDasharray="5 3" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />}
        {data.map((d, i) => {
          if (!d[barAKey]) return null;
          return <circle key={i} cx={groupCenterX(i) - GAP / 2 - BAR_W / 2} cy={barTopA(d[barAKey])} r="2.5" fill={colorA} stroke="white" strokeWidth="1" />;
        })}
        {linePointsB && <polyline points={linePointsB} fill="none" stroke={colorB} strokeWidth="1.5" strokeDasharray="5 3" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />}
        {data.map((d, i) => {
          if (!d[barBKey]) return null;
          return <circle key={i} cx={groupCenterX(i) + GAP / 2 + BAR_W / 2} cy={barTopB(d[barBKey])} r="2.5" fill={colorB} stroke="white" strokeWidth="1" />;
        })}
      </svg>

      {tip && (
        <div className="absolute z-30 pointer-events-none bg-popover border border-border rounded-lg px-3 py-2 shadow-xl text-xs space-y-1"
          style={{ left: Math.min(tip.x + 12, width - 140), top: tip.y }}>
  ...
  return (
    <svg width={140} height={140} viewBox="0 0 120 120">
      {segments.map((seg, i) => {
        ...
        const dash = pct * circ;
        const el = <circle key={i} cx={cx} cy={cy} r={radius} fill="none" stroke={COLORS[i % COLORS.length]} strokeWidth={stroke} strokeDasharray={`${dash} ${circ - dash}`} strokeDashoffset={-offset * circ} style={{ transition: 'stroke-dasharray 0.5s' }} />;
        offset += pct;
        return el;
      })}
  ...
  return (
    <div className="space-y-4 p-1">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Business Statistics</h1>
          <p className="text-muted-foreground text-xs sm:text-sm">Real-time analytics from your invoices</p>
        </div>
      ...
      <div className="flex gap-0 border-b border-border overflow-x-auto scrollbar-hide">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium whitespace-nowrap
      ...
      {activeTab === 'overview' && (
        <Card>
          ...
          <CardContent className="px-2 sm:px-6">
            <ComboChart data={monthlyData} barAKey="revenue" barBKey="count" labelKey="name"
              colorA="#6366f1" colorB="#10b981" labelA="Revenue" labelB="Invoices"
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                ...
                Daily — {FULL_MONTHS[parseInt(selectedMonth)]} {selectedYear}
              </CardTitle>
            </CardHeader>
            <CardContent className="px-2 sm:px-6">
              <ComboChart data={dailyData} barAKey="revenue" barBKey="count" labelKey="name"
                colorA="#6366f1" colorB="#10b981" labelA="Revenue" labelB="Invoices"
                formatA={fmtRevenue} formatB={fmtCount} />
            </CardContent>
          </Card>
      ...
      {activeTab === 'vendor' && <div className="space-y-4"><ShareToggle /><ShareCards data={vendorData} metricLabel="Vendor" /></div>}
      {activeTab === 'brand'  && <div className="space-y-4"><ShareToggle /><ShareCards data={brandData}  metricLabel="Brand"  /></div>}
